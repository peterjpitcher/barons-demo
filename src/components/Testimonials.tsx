...
export function Testimonials() {
  return (
    <section className="relative mx-auto w-full max-w-6xl overflow-hidden rounded-3xl border border-[#314759] text-white shadow-sm">
      <Image
        src={backgroundImage}
      ...
      />
      <div className="absolute inset-0 bg-[#314759]/90" />
      <div className="relative px-4 py-10 sm:px-6">
        <p className="font-display text-sm uppercase tracking-[0.3em] text-[#c1cbd1]">Word of mouth</p>
        <h2 className="mt-2 font-display text-2xl font-semibold">Guests already love the experience</h2>
        <p className="mt-3 max-w-3xl text-sm text-[#e4eaee]">
        ...
        <div className="mt-6 grid gap-4 lg:grid-cols-3">
          {testimonials.map((item) => (
            <figure key={item.quote} className="flex h-full flex-col justify-between rounded-2xl bg-white/10 p-5 backdrop-blur-sm">
              ...
              <figcaption className="mt-4 text-xs font-semibold uppercase tracking-wide text-[#c1cbd1]">
                {item.context}
              </figcaption>
            </figure>
          ))}
